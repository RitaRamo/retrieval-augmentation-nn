...
def create_input_files(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    for img in data['images']:
        ...
        # os.path.join(image_folder, img['filepath'], img['filename']) if dataset == 'coco' else os.path.join(
    ...
    for impaths, imcaps, img_ids, split in [(train_image_paths, train_image_captions, train_image_id, 'TRAIN'),
        ...
        with open(os.path.join(output_folder, split + '_IMGPATHS_' + base_filename + '.json'), 'w') as j:
            json.dump(impaths, j)
